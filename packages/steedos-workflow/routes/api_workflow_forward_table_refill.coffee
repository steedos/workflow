		...
		if req?.query?.oTable
			...
			if req?.query?.oMatchCol
				...
				if columns || columns.length<1
					...
					if d_ins?.form_version == d_ins_form?.current?._id
						...
						d_ins_fields.forEach (d_ins_field)->
							if d_ins_field?.type == 'table' && d_ins_field?.code == d_table
								d_subtable_fields = d_ins_field?.fields
					else
					...
					if o_match_col_field?.type != d_match_col_field?.type
						JsonRoutes.sendResult res, {
							code: 200,
							data: {
								'success': '分发的申请单和原申请单子表的匹配列字段不一致'
							}
						}
						return
					...
					# 附加表格
					if req?.query?.aTable
						a_table_data =  pprove?.values[req?.query?.aTable]
						if a_table_data && a_table_data?.length==table_data?.length
							# 把 附加表的每一列添加到 table_data 中
							a_table_data.forEach (a_row,index)->
								console.log "a_row,index", a_row, index
								# 两个json 合并为一个
								json_obj1 = table_data[index]
								json_obj2 = a_table_data[index]
								json_obj2.forEach (obj)->
									console.log "============================="
									console.log "obj", obj
					...
					console.log "------------------------------------"
					console.log traces[traces.length-1].approves[0].values[o_table]
					...
					JsonRoutes.sendResult res, {
						code: 200,
						data: {
							'success': 'webhook未配置子表回填列字段 columns 值'
						}
					}
					return
			else
				JsonRoutes.sendResult res, {
					code: 200,
					data: {
						'success': 'webhook未配置匹配列字段 oMatchCol 值'
					}
				}
				return
		else
			JsonRoutes.sendResult res, {
				code: 200,
				data: {
					'success': 'webhook未配置原表单子表 oTable 值'
				}
			}
			return
	else
